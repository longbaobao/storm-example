...
public class ProvinceCityRTMapper implements CqlRowMapper<List<String>, Number>, Serializable {
	private static final long serialVersionUID = -6135029143676707055L;

	public static final String KEYSPACE_NAME = "lambdasample";
	public static final String TABLE_NAME = "province_city_count_rt";
	...
	public Statement map(List<String> keys, Number value) {
		...
		statement.value(KEY_NAME_CITY, keys.get(1));
	...
	public Statement retrieve(List<String> keys) {
		Select statement = QueryBuilder.select()
				...
				.column(KEY_NAME_CITY)
				.column(VALUE_NAME)
				.from(KEYSPACE_NAME, TABLE_NAME);
		
		statement.where(QueryBuilder.eq(KEY_NAME_BEGIN_TIME, Util.getTimestampBoundary()));
		statement.where(QueryBuilder.eq(KEY_NAME_PROVINCE, keys.get(0)));
		statement.where(QueryBuilder.eq(KEY_NAME_CITY, keys.get(1)));
